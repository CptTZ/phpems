<?php
/**
3.0或者3.01版本数据库问题的补丁，出现无法注册等情况的使用此补丁尝试。
使用方法：
覆盖lib/pdosql.cls.php文件
*/
//本类主要用于生成各种SQL语句
...
class pdosql
{
	...
	private function _setDefaultInsetValue($type)
	{
		$type = explode('(',$type);
		$type = $type[0];
		switch($type)
	...
	public function makeSelect($args,$tablepre = NULL)
	{
		...
		$db_limits = '';
		if($db_limits == false && $db_limits !== false)$db_limits = 100;
